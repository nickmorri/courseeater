...
returnCourseEvents = function () {
    "use strict";
    var calendarCourses, course, courses, event, days, time, title, start, end, heldDays, i, colors, color, startingDay;
    ...
    for (course in courses) {
        if (courses.hasOwnProperty(course)) {
            ...
            // Random color
            color = colors[parseInt((Math.random() * 5) + 1, 10)];
            ...
            if (time[1].indexOf("AM") == -1) {
                start = parseInt(time[0].split(":")[0], 10) + 12;
                start = start + ":" + time[0].split(":")[1] + ":00";
                start = "T" + start;
                end = time[1].split(" PM")[0];
                end = parseInt(end.split(":")[0], 10) + 12 + ":" + end.split(":")[1] + ":00";
                end = "T" + end;
            } else {
                start = parseInt(time[0].split(":")[0], 10);
                start = "0" + start + ":" + time[0].split(":")[1] + ":00";
                start = "T" + start;
                end = time[1].split(" AM")[0];
                end = "0" + parseInt(end.split(":")[0], 10) + ":" + end.split(":")[1] + ":00";
                end = "T" + end;
            }
